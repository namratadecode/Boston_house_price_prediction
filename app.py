...
import numpy as num  
app = Flask(__name__ )
with open('house_price_prediction.pk1','rb') as f:
    model = pickle.load(f);  
...
def predict():
    ...
    features_array = num.array([features])

    prediction = model.predict(features_array) 
    output = round(prediction[0],2)

    return render_template('index.html',prediction_text=f"Prediction_price: {output}") 
